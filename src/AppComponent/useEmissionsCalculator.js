...
function useEmissionsCalculator() {
    ...
    const getEmissions = async (mode, distanceInMiles, gallonsUsed) => {
        ...
        if (gallonsUsed !== null && distanceInMiles === null) {
            ...
            emissions = await axios.post("https://ecowiseus.herokuapp.com/getEmissions", body)
    ...
    const getVehicleMPG = async (carModeInfo) => {
        ...
        try {
            mpg = await axios.post("https://ecowiseus.herokuapp.com/getVehicleMPG", carModeInfo)
        } catch (error) {
            console.log(error)
